...
export class InstructorProfilePageComponent implements OnInit {
  ...
  edit_course_dateovr : string;
  edit_course_datefrm : string;
  edit_course_nbh : string;
  video_link : string;
  ...
  new_course_dateovr : string;
  new_course_datefrm : string;
  new_course_nbh: string;
  alert: boolean;
  ...
  changeCourseDetails(name : string, descrp : string, author : string, tag : string, chapter : string,
                      test : string,video : string, image : string, dateovr:string, datefrm:string, nbh:string 
                      )
  {
    ...
    post_course.dateovr = dateovr;
    post_course.datefrm = datefrm;
    post_course.nbh = nbh;
  ...
  addNewCourse(name : string, descrp : string, author : string, tag : string, chapter : string,
                      test : string, video : string, image : string, rating : string, dateovr:string,datefrm:string,nbh:string)
  {
    ...
    let post_course : Course = new Course(0,code,name,author,descrp,rating,tag,test,chapter,
      video,image,0,date,date,dateovr,datefrm,nbh);
  ...
  editSelectedCourse(course : string)
  {
    ...
    this.edit_course_dateovr = this.selected_course.dateovr;
    this.edit_course_datefrm = this.selected_course.datefrm;
    this.edit_course_nbh = this.selected_course.nbh;
  ...
  {
    this.iProfileService.deleteCourse(id).subscribe(
      (data :string) => {
        alert(data);
      }
    );
  ...
  public getDatabase()
  {
    new Promise( (resolve,reject) => {
      setTimeout( () => {
        try {
          this.courseList = [];
          this.iProfileService.getAllCourses().subscribe(
            (data: Course[]) => {
