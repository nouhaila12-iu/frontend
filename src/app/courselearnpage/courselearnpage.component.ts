          ...
          try {
            this.learnService.getCourseById(this.course_id).subscribe(
              (data: Course) => {
                ...
                if(!user.enrolled_courses.includes(this.course_code))
                {
                  user.enrolled_courses = user.enrolled_courses + this.course_code + ',';
                  this.learnService.updateEnrolledCourse(user).subscribe((data: User) => {
                    sessionStorage.setItem('current_user', JSON.stringify(data));
                  });
                }
              }
